"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
exports.__esModule = true;
exports.AuthorizationException = void 0;
var AuthorizationException = /** @class */ (function (_super) {
    __extends(AuthorizationException, _super);
    function AuthorizationException() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    AuthorizationException.wrongCredentials = function () {
        return new AuthorizationException('Неверный email или пароль');
    };
    AuthorizationException.blocked = function () {
        return new AuthorizationException('Пользователь заблокирован');
    };
    AuthorizationException.refreshTokenNotFound = function () {
        return new AuthorizationException('Refresh token not found');
    };
    AuthorizationException.badRefreshToken = function () {
        return new AuthorizationException('Bad refresh token');
    };
    return AuthorizationException;
}(Error));
exports.AuthorizationException = AuthorizationException;
